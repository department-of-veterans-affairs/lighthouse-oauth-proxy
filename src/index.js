...
function startApp(config, isolatedIssuers) {
  ...
  if (config.routes && config.routes.categories) {
    Object.entries(config.routes.categories).forEach(([, app_category]) => {
      ...
      if (app_category.previous_app_category_settings && app_category.previous_app_category_settings.upstream_issuer) {
        app_category.previous_app_category_settings.okta_client = new okta.Client({
          orgUrl: config.okta_url,
          token: config.okta_token,
          requestExecutor: new okta.DefaultRequestExecutor(),
        });
    ...
    try {
      ...
      if (config.routes && config.routes.categories) {
        for (const app_category of config.routes.categories) {
          ...
          if (app_category.previous_app_category_settings && app_category.previous_app_category_settings.upstream_issuer) {
            app_category.previous_app_category_settings.issuer = await createIssuer(app_category.previous_app_category_settings);
          }
        }
      }
...
const proxyRequest = async (
  ...
  let redirectUrl = clientScreenedProxyRequest.previous_app_category_settings
    ? clientScreenedProxyRequest.previous_app_category_settings.issuer.metadata[metadata_type]
  ...
  axios(proxy_request)
    ...
    .catch((err) => {
      ...
      if (api_category && api_category.previous_app_category_settings) {
        proxy_request.url = api_category.previous_app_category_settings.issuer.metadata[metadata_type];
        axios(proxy_request)
          .then((response) => {
            setProxyResponse(response, res);
          })
