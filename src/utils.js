...
const screenForV2ClientId = async (client_id, dynamoClient, config, path) => {
  ...
  if (apiCategory && apiCategory.previous_app_category_settings && apiCategory.previous_app_category_settings.upstream_issuer) {
    try {
  ...
  if (
    ...
    apiCategory &&
    apiCategory.previous_app_category_settings
  ) {
    v2transitiondata.previous_app_category_settings = apiCategory.previous_app_category_settings;
  }
...
const v2TransitionReqRewrite = async (req, dynamoClient, config) => {
  ...
  req.previous_app_category_settings = v2transitiondata.previous_app_category_settings;
  return req;
};
